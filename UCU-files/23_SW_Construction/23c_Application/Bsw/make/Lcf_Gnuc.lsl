...
LCF_TRAPVEC0_START = 0x80080100;
LCF_TRAPVEC1_START = 0x801F6100;
...
MEMORY
{
  ...
  pfls2 (rx!p): org = 0x80400000, len = 2M        /*Not used to allocate and sections*/
  pfls2_nc (rx!p): org = 0xa0400000, len = 2M     /*Not used to allocate and sections*/

  pfls3 (rx!p): org = 0x80600000, len = 2M        /*Not used to allocate and sections*/
  pfls3_nc (rx!p): org = 0xa0600000, len = 2M     /*Not used to allocate and sections*/
	...
	edmem (w!xp): org = 0x9f000000, len = 2M
	edmem_nc (w!xp): org = 0xbf000000, len = 2M

	edmem_xtm (w!xp): org = 0x9f200000, len = 16K
	edmem_xtm_nc (w!xp): org = 0xbf200000, len = 16K
}
...
*/
  ...
  /*This section is always required as Boot mode header 2 address absolutely restricted at address 0x8000FFE0*/
/* used in Bootloader   
  .bmhd_2 (0x8000FFE0) : FLAGS(arl)
  {
      BootModeHeader2 = .;
      KEEP (*(.bmhd_2));
  } > pfls0
*/
  ...
  /*This section is always required as Boot mode header 3 address absolutely restricted at address 0x8001FFE0*/
/* used in Bootloader   
  .bmhd_3 (0x8001FFE0) : FLAGS(arl)
  {
      BootModeHeader3 = .;
      KEEP (*(.bmhd_3));
  } > pfls0
*/  
  ...
  /*This section contains the jump to the actual begin of startup code */
  .startupindo (0x80080000) :
  {
      KEEP (*(.startupindi));
      . = ALIGN(4);
  } > pfls0
    ...
    *(.srodata.*)
   	...
   	.rodata : FLAGS(arl)
	{
    	...
    	*(.gnu.linkonce.r.*)
    	...
    	LONG(0 + ADDR(.bss_emem_xtm)); LONG(SIZEOF(.bss_emem_xtm));
    	LONG(-1);                      LONG(-1);
    	...
    	LONG(LOADADDR(.data_lmu_share));   LONG(0 + ADDR(.data_lmu_share));    LONG(SIZEOF(.data_lmu_share));
    	LONG(LOADADDR(.data_emem_xtm));    LONG(0 + ADDR(.data_emem_xtm));     LONG(SIZEOF(.data_emem_xtm));
...
SECTIONS
{
  ...
  /* ToDo GRSC locate to LMU RAM */
	.data_lmu_share 0xb0000000 (NOLOAD) : FLAGS(awl)
	{
    KEEP (*(.CpuShareSection))
    KEEP (*(.CpuShareSection.*))
	} > lmuram_nc 
    ...
    *(.lmubss.*)
	...
	.data_emem_xtm : FLAGS(awl)
	{
		. = ALIGN(4) ;
    	*(.data_emem_xtm)
    	*(.data_emem_xtm.*)
    	*(.gnu.linkonce.d.*)
	} > edmem_xtm_nc AT> pfls0
...
/* Used as calibration ROM */
	.rodata_calib (0x801FC000) : FLAGS(arl)
	{
		KEEP (*(.rodata_calib))
	} > pfls0
}
